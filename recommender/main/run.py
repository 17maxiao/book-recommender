# Book Recommendation Engine by Cristian Alberch
...
def query_book_ratings(query_dict):
    ...
    '''
    INPUT:
    dictionary of 60 books. key: book_id; and value: book title.
    
    OUTPUT:
    returns a dictionary with key: book_id; and value: user book rating
    '''
...
def query_genre_likes(genres):
    '''
    INPUT:
    - genres: dictionary of genres options for user to choose from. \
        key: sequential number, value: genre.
    OUPUT:
    - genres: list with user's selection of genres.
    '''
...
def weighted_mean(df_ratings, user_ratings, sample_size = 100):
    ...
    '''   
    INPUT:
    - df_ratings: dataframe containing book ratings. 
        Ratings arranged by columns: user_id; book_id; rating.
    - user_ratings: dictionary containing user's book ratings. 
        Arranged by book_id (key) and rating (value).
    - sample_size: integer. 
        Size of random sample from all the readers' ratings.
    
    OUTPUT:
    - book_wmeans: dictionary containing weighted means for every book \
    ...
    # create a copy of unstacked dataframe filling all NaN values with zero's.
    ...
    # create a dataframe from the cosine_ratings matrix. 
    ...
    # get a series containing the cosine similarity between User (row 0) 
